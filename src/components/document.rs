        ...
        View(flex_grow: 1.0, border_style: BorderStyle::Single){
                ScrollView {
                    ...
                    #(doc.blocks.iter().map(|block| match block {
                        ...
                        Block::Mermaid { source } => element!{MermaidBlock(source: source.clone())}.into_any(),
                        Block::Math { content, display } => element!{MathBlock(content: content.clone(), display: *display)}.into_any(),
