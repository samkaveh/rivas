...
use crate::components::editor::{Mode, EditorState};
                        ...
                        View(
                            ...
                            #(lines.iter().enumerate().map(|(idx, line)| {
                                ...
                                while !remaining.is_empty() {
                                    let split_at = remaining.char_indices().nth(wrap_width).map(|(i, _)| i).unwrap_or(remaining.len());
                                        ...
                                        #(segments.iter().enumerate().map(|(seg_idx, segment)| {
                                            if mode == Mode::Visual {
                                                if let (Some(start), Some(end)) = (vis_start, vis_end) {
                                                    ...
                                                    element! { Text(content: segment.to_string(), color: crate::theme::FG, wrap: TextWrap::NoWrap) }.into_any()
                                                }
                                                ...
                                                if seg_idx == seg_idx_cursor {
                                                    let (before, after) = segment.split_at(seg_rel_off.min(segment.len()));
                                                    element! {
                                                        View(flex_direction: FlexDirection::Row) {
                                                            Text(content: before, color: crate::theme::FG, wrap: TextWrap::NoWrap)
                                                            View(background_color: cursor_bg, width: 1) {
                                                                Text(content: " ", color: crate::theme::DARK_BG, wrap: TextWrap::NoWrap)
                                                            }
